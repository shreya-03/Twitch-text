...
def get_additonal_k_split(K, X, clst_labels, clst_centers, n_features, K_sub, k_means_args):
	...
	for clst_index in range(K):
		...
		for i in range(len(X)):
			if clst_labels[i] == clst_index:
				clst_points.append(X[i])
		clst_points = np.array(clst_points)
		...
		for subclst_index in range(K_sub):
			...
			subclst_points = []
			for i in range(len(clst_points)):
				if subclst_labels[i] == subclst_index:
					subclst_points.append(clst_points[i])
			subclst_points = np.array(subclst_points)
...
class XMeans(KMeans):
	def __init__(self, kmax=10, max_iter=1000, **k_means_args):
		"""
